...
CTextureManager MSOTextureManager;

CTextureManager &GetTextureManager(){
    return MSOTextureManager;
}
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'n':               /* newmtl */
			...
			model->materials[nummaterials].texture = NULL;
			model->materials[nummaterials].normaltexture = NULL;
            break;
        case 'N':
        ...
        fgets(buf, sizeof(buf), file);
        sscanf(buf, "%s %s", buf, buf);
		printf("to CreateTexture : %s\n",buf);//<<<<
		tempTex = GetTextureManager().CreateTexture(buf);
		if(tempTex){
			//printf("model->materials[nummaterials]:%d tempTex:%d is true\n",nummaterials,tempTex);//<<<<
			model->materials[nummaterials].texture = tempTex;
		}
		...
		tempTex = GetTextureManager().CreateTexture(buf);
		if(tempTex){
			model->materials[nummaterials].normaltexture = tempTex;
		}
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
		...
		//if(group->next)
		//{
		//	printf("now:%s next:%s\n",group->name,group->next->name);
		//}
		//int i=0;

		//for(int i=0 ; i<=19 ; ++i)
		//{
		//	printf("%d : %d \n",i,(model->materials[i].texture));
		//}
		...
		if(mode & GLM_TEXTURE){
			if(group->material && model->materials[group->material].texture)
				GetTextureManager().BindTexture((model->materials[group->material].texture));
			if(group->material && model->materials[group->material].normaltexture)
				GetTextureManager().BindNormalTexture((model->materials[group->material].normaltexture));	
		}
